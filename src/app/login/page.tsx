  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-background relative overflow-hidden">
      ...
      <div className="hidden md:flex flex-1 bg-slate-950 relative overflow-hidden flex-col items-center justify-center p-12">
        ...
        >
          <div className="w-20 h-20 bg-teal-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-teal-600/20 mb-8 mx-auto rotate-3">
            <Brain className="h-10 w-10 text-white" />
          </div>
          <h2 className="text-4xl font-black text-white mb-6 tracking-tight">Intelligence at <br /> Every Step.</h2>
          <p className="text-slate-400 text-lg max-w-sm mx-auto leading-relaxed">
            Our multi-agent system is ready to analyze your profile and build your roadmap.
          </p>
        </motion.div>
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-2">
          {[1, 2, 3].map(i => (
            <div key={i} className={`w-2 h-2 rounded-full ${i === 1 ? 'bg-teal-500' : 'bg-slate-800'}`} />
          ))}
        </div>
      </div>
